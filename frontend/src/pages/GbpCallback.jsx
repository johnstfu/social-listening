...
export default function GbpCallback() {
  ...
  useEffect(() => {
    ...
    axios.get(`${API_URL}/api/oauth/gbp/callback?code=${code}&state=${state || 'default'}`)
      .then(res => {
  ...
  const handleSelectAccount = (accountName) => {
    ...
    axios.get(`${API_URL}/api/oauth/gbp/locations?access_token=${token}&account_name=${accountName}`)
      .then(res => {
  ...
  const handleSelectLocation = async (location) => {
    ...
    try {
      ...
      const response = await axios.post(`${API_URL}/api/oauth/save-connection`, null, {
        params: {
      ...
      setStep('done')
      setTimeout(() => {
        navigate('/connectivity?connected=gbp')
      }, 2000)
    } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-teal-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl p-8 text-center max-w-md">
          ...
          <h2 className="text-xl font-semibold text-slate-800 mb-2">Erreur de connexion</h2>
          <p className="text-red-600 mb-4">{error}</p>
          <button
            onClick={() => navigate('/connectivity')}
            className="px-6 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition"
          >
            Retour à Connectivité
          </button>
        </div>
      </div>
    )
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-teal-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl p-8 max-w-lg w-full">
          ...
          <h2 className="text-xl font-semibold text-slate-800 mb-2 text-center">Sélectionnez un compte</h2>
          ...
          <div className="space-y-3">
            {accounts.map((account, index) => (
              <button
                ...
                className="w-full p-4 border border-slate-200 rounded-xl hover:border-emerald-500 hover:bg-emerald-50 transition text-left"
              >
                <div className="font-medium text-slate-800">{account.accountName || account.name}</div>
  ...
  if (step === 'select_location') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-teal-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl p-8 max-w-lg w-full max-h-[80vh] overflow-y-auto">
          ...
          <h2 className="text-xl font-semibold text-slate-800 mb-2 text-center">Sélectionnez votre établissement</h2>
          ...
          <button
            onClick={() => navigate('/connectivity')}
            className="w-full mt-4 px-6 py-2 border border-slate-300 text-slate-600 rounded-lg hover:bg-slate-50 transition"
          >
            Annuler
          </button>
        </div>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-teal-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl p-8 text-center max-w-md">
          ...
          <h2 className="text-xl font-semibold text-slate-800 mb-2">Connexion réussie!</h2>
